...
class KodiRPC:
    ...
    def update_rp(self, info, length):
        try:
            self.set_rp(info, length)
        except PipeClosed:
    ...
    def fetch_info(self):
        while True:
            try:
                response = self.session.get(INFO_URL).json()
                return response
    ...
    def fetch_length(self):
        while True:
            try:
                response = self.session.get(LENGTH_URL).json()
                return response
            except requests.exceptions.RequestException as e:
                logger.error(f"Can't connect to Kodi web interface: {e}. Are you sure it's running? Is the web interface on?")
                time.sleep(2)
    ...
    def set_rp(self, info, length):
        ...
        if length['speed'] != 0:
            start_time = self.calculate_start_time(length)
            end_time = self.calculate_end_time(start_time, length, self.session)
            logger.debug(f"Calculated start_time={start_time} and end_time={end_time}")

        self.update_rpc_mediatype(self, info, length, start_time, end_time, image_url, imdb_url, tmdb_url, trakt_url, letterboxd_url)

        previous_info = info
        previous_speed = length['speed']
        
    # Function to update the RP based on the media type
    def update_rpc_mediatype(self, info, length, start_time, end_time, image_url, imdb_url, tmdb_url, trakt_url, letterboxd_url):
        media_type_functions = {
            'movie': self.update_rpc_movie,
            'episode': self.update_rpc_episode,
            'channel': self.update_rpc_channel,
            'unknown': self.clear_rpc_if_unknown
        }
    ...
    def clear_rpc_if_unknown(self, info, length, start_time, end_time, image_url, imdb_url, tmdb_url, trakt_url, letterboxd_url):
        if length['speed'] == 0:
    ...
    def update_rpc_movie(self, info, length, start_time, end_time, image_url, imdb_url, tmdb_url, trakt_url, letterboxd_url):
        is_playing = length['speed'] != 0
        self.update_rpc(info, start_time, end_time, image_url, imdb_url, tmdb_url, trakt_url, letterboxd_url, 'movie', is_playing)

    # Function to update the RP for an episode
    def update_rpc_episode(self, info, length, start_time, end_time, image_url, imdb_url, tmdb_url, trakt_url, letterboxd_url=None):
        is_playing = length['speed'] != 0
        self.update_rpc(info, start_time, end_time, image_url, imdb_url, tmdb_url, trakt_url, None, 'episode', is_playing)

    # Function to update the RP for a channel
    def update_rpc_channel(self, info, length, start_time, end_time, image_url, *_):
        is_playing = length['speed'] != 0
        self.update_rpc(info, start_time, end_time, image_url, None, None, None, None, 'channel', is_playing)

    # Function to update the RP
    def update_rpc(self, info, start_time, end_time, image_url, imdb_url, tmdb_url, trakt_url, letterboxd_url, media_type, is_playing):
        logger.info(f"{UPDATED_RPC} - {'Playing' if is_playing else 'Paused'} {media_type.capitalize()}:")
        logger.info(f"{info['title']}")

        buttons = self.create_buttons(imdb_url, letterboxd_url, tmdb_url, trakt_url)
        ...
        rpc_params = {
            ...
            "large_text": self.large_text_map.get(media_type.lower(), "Default Large Text"),
    ...
    def calculate_start_time(self, length):
    ...
    def calculate_end_time(self, start_time, length, session):
        ...
        if start_time == end_time:
            new_length = self.fetch_info()  # Refetch the session
            new_start_time = self.calculate_start_time(new_length)  # Recalculate the start time
            return self.calculate_end_time(new_start_time, new_length, session)  # Recalculate the end time with the new start time and length

        return end_time

    # Function to create the buttons for a media
    def create_buttons(self, imdb_url, letterboxd_url, tmdb_url, trakt_url):
        buttons = []
        button_data = [
    ...
    def get_urls(self, info, media_type):
        trakt_url = None
        tmdb_url = None
        imdb_url = None
        letterboxd_url = None
